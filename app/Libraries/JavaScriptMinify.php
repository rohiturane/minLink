...
class JavaScriptMinify
{
    ...
    {
        $java_special_chars = array($this->blockCommentFinder, // JavaScript Block Comment
        ...
        while ($sequence = $this->getNextSpecialSequence($javascript, $java_special_chars)){
            switch ($sequence->type){
                ...
                case '//':// remove comments
                    $javascript = substr($javascript, 0, $sequence->start_idx) . substr($javascript,
                    $sequence->end_idx);
                break;
                default: // quoted strings or regex that need to be preservered
                    ...
                    $this->minificationStore[$placeholder] =substr($javascript, $start_idx, $end_idx -
                    $start_idx);
                    $javascript = substr($javascript, 0, $start_idx) . $placeholder . substr(
                    $javascript, $end_idx);
            }
        }
